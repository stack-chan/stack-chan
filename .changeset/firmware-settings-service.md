...
Introduce bounded, framed BLE settings protocol 2 with save acknowledgements and explicit application timing. Use matching Web tools with the new firmware. The updated Web tools keep sends to older firmware marked unconfirmed, preserve untouched secrets, and derive available setting choices from the firmware contract. Unknown setting keys and invalid values are rejected; MOD-provided Wi-Fi defaults are no longer used.
