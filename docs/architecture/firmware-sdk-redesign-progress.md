...
**2026-09-09 完了条件の更新：製品ソースの純減は要求しない。** 明示的な検証・取消し・所有管理に必要なコード増加を許容し、F11は廃止したアプリAPIの実装・利用者・互換経路が残っていないことを基準とする。行数は参考値として記録する。以下の日付付き記録の「純減未達」は当時の条件による記述であり、現在の残件には数えない。最新の撤去範囲と検証は [旧APIの残存確認](retired-api-audit-2026-09-09.md) を参照する。
...
通信・会話・ストリーミング音声・SHT3x・サーボ保守・設定を型付き拡張として接続した。これらの14例の最小host APIは7。機器と購読はAppSessionに所有させ、音声の占有はRuntimeAudio、保守は設定済みの同じサーボバスを使う。旧MOD内の直接生成・raw参照・重複する役割と、利用者がなくなったWhisper実装・MOD専用chat設定処理を削除した。

ソース移行・自動試験と、実機・初学者の受入は分ける。V1ホスト／Blockly／参考providerライブラリーの整理、全無線方式の調停、電源断・保存操作の実機受入、製品ソース純減は残件である。今回の変更で F1〜F12 全体の完了を宣言しない。契約・検証結果・コード量は [今回の移行記録](sdk-example-migration-2026-09-08.md) を参照する。

| 課題 | 必要な最終状態 | 状態・検証先 |
| --- | --- | --- |
| F1 公開境界 | V2 の SDK は旧 flat API、具体的 TTS・sensor・Piu controller を含まない。高度な拡張を別入口にする | 基本SDKとPiu・UI・入力・lighting・画像・通信・会話・ストリーミング音声・センサー・保守・設定の公開拡張を接続。全実行例が公開入口のみを使用。V1ホストの公開面撤去は残る |
...
| F4 競合と重複 | 音声、会話、USB、motion、物理 UART の資源管理を共通化。上限・期限・取消しを保証 | 通常音声・録音とradio／realtime／USBの占有をRuntimeAudioで調停。motionとサーボ保守は設定済みの同じバスを使用。全無線方式の排他、V1利用者の撤去と実機受入は未完了 |
| F5 教材適合 | 全 MOD／miniapp の入口を分類・移行。公開契約に適合し、機種差の回避策を基盤へ移す | 元の32例を21個のAPI 2パッケージへ移行・統合。実行例のAPI 1は0。全例のstrict・import境界・archiveビルドを検査。機器・初学者の受入は未完了 |
| F6 アプリ構成 | 既定動作、診断、UI 拡張の責務と寿命を分ける。既定動作にも SDK と AppSession を使用 | 既定動作・診断・Piu画面もSDKとAppSessionを使用。旧フック継承・miniapp専用起動を撤去。V1ホストとBlocklyの旧実行経路は残る |
| F7 正本 | 共通 manifest、ボード設定、公開型と module exports の正本を統一。target 別の型検査を成立させる | 公開設定schemaをstackchan/settings-schemaへ集約し、本体・Web・SDKで共有。公開拡張と利用例のstrict検査を接続。ボード別公開型の整理は残る |
| F8 設定・起動 | 型・検証・優先順位・secret・適用時点を共通設定サービスへ集約。オフライン教材を Wi-Fi 待機から独立 | オフライン教材とWi-Fi待機を分離。SettingsServiceを本体・BLE・Web・アプリSDKへ接続。chat設定の正本と適用情報を統一。V1起動全体と電源断時の受入は残る |
...
| F10 検査・配布 | 公開依存・JS / TS 教材・型・ライフサイクルを実効的に検査。V2 metadata を Web / CLI / SD / WASM で起動前検証 | 全21例・7教材をAPI 2へ統一し、CLI・Web・SD・WASMで宣言を事前検査。CIに全例のarchiveビルドを追加。WASMと3機種をビルド。実機受入は未完了 |
...
## 設定の旧形式撤去と一覧の準備完了（2026-09-07）

対象ソースは `93df6fb56151ea425fd7fae440771366284485cb`。未使用の `PREF_KEYS` tupleと `renderer.type` の読み替え・書き戻しを削除した。顔は `ui.type` だけで指定し、設定の読み込みは旧値を永続化しない。

本体は設定一覧の全項目を送った後でreadyを通知する。Webは同じprotocol・全管理キー・readyを5秒以内に受信してから一覧を公開し、接続完了とする。設定読み込み失敗・不完全な一覧・切断・タイムアウトでは保存を開始しない。受信は改行フレームだけを解釈し、旧ファームへの未確認送信と単一項目のprop/value要求を撤去した。保存は正のrequestIdで対応する本体応答を待ち、成功時は適用時点と適用失敗フラグを必ず返す。保存失敗では編集を残し、保存済みだが即時適用に失敗した場合は再保存と区別して再起動を案内する。
...
## 画像アバターの SDK 化と配置・表情の修正（2026-09-07）

対象ソースは `be3ddc0e1cf436ff37c92a191ca5db12fce1b2b4`。`image_avatar_lite` を同じAppSessionの `ui(app).setImageAvatar(pack)` へ移した。画像パックの公開型とSDKの表情名を正本とし、ホスト側の数値で大文字文字列のmapを検索していた不一致を解消した。目と口は静止画像の項目に `frameCount` を加える同じ形式とし、画像名・フレームサイズの二重指定と二種類の描画更新処理をまとめた。

グローバル登録・パックID検索と暗黙fallback・`ui.avatar`・サンプルの内部 `parts/*` importを撤去した。名前・サイズ・表情の対応・フレーム数を検証して深いコピーを凍結し、全表情の素材を準備してから顔を置換する。未使用の表情のPNG欠落も選択時に失敗し、現在の顔を残す。アプリ終了は既存のappearance所有処理で設定済みの顔を復元し、メニューと入力も解除する。別のレジストリーや実行器は追加していない。

画面確認で、小さな顔の左上位置を大きなアバターへ引き継ぐため右端と口が切れる問題も確認した。共通の顔切り替えを中心位置の維持へ直し、clipの余白・奇数と偶数のサイズ変更・小さい顔への復元で位置がずれないことを検査する。修正後のブラウザーでも口を含む全体の表示を確認した。元の6キャラクター・12表情・43画像について、直前の `eb05450` のデータから表情名と重複sheet項目だけを正規化した比較で配置・素材・割り当ての同等性を確認した。素材とライセンスは変更していない。

検証: Node全537件、構成78件、SDK strict、6対象のmanifest検査、XS全60 manifest（37.9秒）が成功。XSで実Piuの表情・フレーム切り替えとskin再利用、入力オブジェクトの後変更からの分離、PNG欠落・不正入力での表示保持、SDK終了後の拒否、配置の往復を確認した。Webの77件と旧ランナー258件、WASM・Webビルドが成功。Chromiumでは12教材・例、6キャラクター全体の切り替え、既定アプリ、音声・録音・カメラの解放、MOD事前検査・復旧と設定を閉じてからのMOD評価が成功した。途中の通し検査ではtoneの完了待ちが一度タイムアウトした。追加した画素比較を画像アバター実行時だけに限定し、ブラウザー検査を順番に実行した最終版で通過を確認した。

nativeのreleaseはCoreS3 6,625,232 bytes、Takao Core2 SG90 3,927,776 bytes、Stackchan RT 4,048,432 bytes。3機種とも `9.5.0+stackchan.6` で、直前より12,288 bytes増えた。host API 6の宣言とmajor changeset、SDK契約、1ファイルの改造・エラー復帰手順、CIのブラウザー対象を更新した。ソース行数の縮小をバイナリー容量や実機ヒープの縮小とは扱わない。

固定規則による製品測定は472ファイル・59,432物理行（直前より1ファイル増・51行減）。内訳はfirmware実装85行減、公開SDK・契約34行増。サンプルは別計上で681行減、試験・補助は314行増。画像記述の共通化によるサンプル側の減少は製品の削減に含めず、削除した製品処理をサンプルや生成物へ移してもいない。起点に対しては53ファイル・6,730行増で、全体の純減は未達。

元の32例のうち11例をSDKの7パッケージへ移行・統合し、21例は未移行。Webの顔エディター（ShapeFace）とBlockly、V1起動と旧名前空間・raw型、公開設定APIとmod/config、会話・USB・音声の資源調停、全能力の事前検査、実機での表示・メモリー・操作と初学者受入は未完了。F1〜F12全体の完了とはしない。
