...
当時の製品純減条件による計測を以下に保持する。2026-09-09の方針更新後は行数を参考値とし、[旧APIの残存確認](retired-api-audit-2026-09-09.md) を完了条件とする。項目1の時点では487ファイル・60,991物理行で、起点 `6eb4623` より68ファイル・8,289行増。サンプルや試験への分類変更、コメント削除、コード圧縮を削減実績に数えない。

## コミットと再計測

実装は `885c8b90a072d626214bda7bb4cc805d991e83b8`。固定した `measure.py` による製品ソースは **484ファイル・59,775物理行**。直前のBlockly移行から **3ファイル・1,216行減**。内訳はfirmware実装1,231行減、公開契約21行増、Web実装7行減、型宣言1行増である。サンプルは別計上で2ファイル・3行減、試験・補助は4ファイル・503行減。生成物や教材への付け替えによる削減ではない。

起点からは依然 **65ファイル・7,073行増**。V1撤去による改善と再設計全体の純減達成を混同しない。

| release対象 | bytes | 埋込版 |
| --- | ---: | --- |
| M5StackChan CoreS3 | 6,686,672 | 9.5.0+stackchan.9 |
| Takao Core2 SG90 | 3,989,216 | 9.5.0+stackchan.9 |
| Stackchan RT | 4,109,872 | 9.5.0+stackchan.9 |

3対象とも直前より24,576 bytes減少。バイナリー容量・ソース行数の減少を実機ヒープの改善とは扱わない。
