...
F2 / F3 / F9のWASM出力を扱う。音声出力の完了は、Web Audioの終了イベントと資源の解放を確認した時点とする。指定時間が経過しただけでtoneを成功にする経路を廃止する。現在公開している再生操作のAppSessionへの接続は[アプリの音声所有](app-audio-lifecycle.md)を参照。公開録音・バッファ再生SDK、会話やUSBとの調停は引き続き残る。
