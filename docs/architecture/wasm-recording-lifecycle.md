...
F2 / F3 / F9の録音経路を扱う。公開SDKの録音・バッファ再生APIとそのAppSessionへの接続、会話と音声出力の調停は後続の作業であり、この変更だけで完了とはしない。公開済みの発話・素材再生・toneの所有は[AppSessionの音声操作](app-audio-lifecycle.md)を参照。
