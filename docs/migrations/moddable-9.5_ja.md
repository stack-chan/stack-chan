...
M5StackChan CoreS3の追加実機検証では、HTTPとMCP、100回の接続と切断、73,728バイトのHTTP応答、過大な本文の拒否、12 KBの追加ヘッダーを含むWebSocket Upgrade、未読データを残した切断が成功しました。
NTPによる時刻取得、保存済みWi-Fiへの自動再接続、再接続後のHTTPS応答も確認しています。
ChatAudioIOは30分動作し、173回の音声往復を処理しましたが、通信の遅延については追加調査が必要です。
Web Radio専用アプリも30分再生し、再生エラー、再接続、クラッシュはありませんでした。
条件と制限は[実機検証記録](../investigations/moddable-9.5-hardware-2026-09-06.md)を参照してください。
...
実際の音質、画面、サーボの動き、USB音声の手動確認は別途必要です。
