...
M5Stack版StackChan CoreS3 を標準構成として扱います。
初めての場合は次の順で進めてください。
...
M5Stack Core2 など別の本体では `npm run flash:core2` のような名前付きコマンドを使います。
MOD の開発だけを繰り返す場合は `npm run mod -- mods/look_around/manifest.json` のように MOD の manifest を指定します。
