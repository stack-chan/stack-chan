...
/** @typedef {'live' | 'reconnect' | 'restart'} SettingApplication */
export const SETTINGS_PROTOCOL_VERSION = 2
export const SETTINGS_MESSAGE_MAX_BYTES = 32_768
/** @typedef {{applications: readonly SettingApplication[], applyFailed: boolean}} SettingsSaveReceipt */
/** @template {string | number} T
 * @typedef {Readonly<{
 * kind: 'string' | 'number', defaultValue: T | undefined, optional: boolean,
 * secret: boolean, application: SettingApplication, appDefault: boolean,
 * choices?: readonly string[], minimum?: number, maximum?: number,
 * integer?: boolean, maxBytes?: number
 * }>} Definition */

/** @param {string | undefined} defaultValue @param {number} maxBytes
 * @param {boolean} [secret] @returns {Definition<string>} */
function text(defaultValue, maxBytes, secret = false) {
...
/** @param {number | undefined} defaultValue @param {number} minimum @param {number} maximum
 * @param {boolean} [integer] @param {SettingApplication} [application] @returns {Definition<number>} */
function number(defaultValue, minimum, maximum, integer = false, application = 'restart') {
  return Object.freeze({
    kind: 'number',
    defaultValue,
    minimum,
...
/** @template {string} T
 * @param {T} defaultValue @param {readonly T[]} choices
 * @param {SettingApplication} [application] @returns {Definition<T>} */
function choice(defaultValue, choices, application = 'restart') {
  return Object.freeze({
    kind: 'string',
    defaultValue,
...
/** @typedef {(typeof TIMEZONE_PRESETS)[number]['id']} TimezoneId */
/** @typedef {(typeof TIMEZONE_PRESETS)[number]} TimezonePreset */
/** @type {TimezoneId} */
export const DEFAULT_TIMEZONE_ID = 'tokyo'

export const SETTINGS_SCHEMA = Object.freeze({
  'wifi.ssid': Object.freeze({ ...text('', 32), application: /** @type {const} */ ('reconnect'), appDefault: false }),
  'wifi.password': Object.freeze({
    ...text('', 64, true),
    application: /** @type {const} */ ('reconnect'),
    appDefault: false,
  }),
  ...
  'chat.type': choice('openAIRealtime', [
    'deepgramAgent',
    'elevenLabsAgent',
    'googleGeminiLive',
    'humeAIEVI',
  ...
  'chat.apiKey': text('', 2_048, true),
  'chat.endpoint': text(undefined, 2_048),
  'chat.modelID': text(undefined, 128),
  'chat.voiceID': text('marin', 128),
  'chat.instructions': text(undefined, 2_048),
 ...
 * @typedef {(typeof SETTINGS_SCHEMA)[K] extends Definition<infer T> ? T | undefined : never} SettingValue */
/** @typedef {keyof typeof DOMAIN} SettingDomain */
/** @template {SettingDomain} D
 * @typedef {{[K in SettingKey as K extends `${D}.${infer Name}` ? Name : never]?: SettingValue<K>}} SettingsForDomain */
export const SETTING_KEYS = Object.freeze(/** @type {SettingKey[]} */ (Object.keys(SETTINGS_SCHEMA)))

/** @param {string} value @returns {value is SettingKey} */
...
/** @typedef {{valid: true, value: string | number | undefined} | {valid: false, message: string}} SettingValidation */

/** Numeric strings support existing NVS/BLE values; empty optional fields reset to defaults.
 * @param {SettingKey} key @param {unknown} input @returns {SettingValidation} */
