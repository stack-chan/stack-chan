...
test('renderStackchanVoiceWav renders 24 kHz mono PCM with volume and a valid WAV header', () => {
  const voice = new FakeStackchanVoice(new Int16Array([1000, -1000, 2000]))

  const rendered = renderStackchanVoiceWav(voice, 'こんにちは', {
    chunkSamples: 2,
    speed: 120,
    volume: 0.5,
  })
...
test('renderStackchanVoiceWav grows its PCM buffer and clamps volume', () => {
  const samples = Int16Array.from({ length: 6000 }, (_, index) => (index % 2 === 0 ? 20000 : -20000))
  const voice = new FakeStackchanVoice(samples)

  const rendered = renderStackchanVoiceWav(voice, '長い文章', { chunkSamples: 257, volume: 2 })
...
test('renderStackchanVoiceWav emits an empty but valid WAV for an empty utterance', () => {
  const rendered = renderStackchanVoiceWav(new FakeStackchanVoice(new Int16Array()), '', { volume: -1 })
