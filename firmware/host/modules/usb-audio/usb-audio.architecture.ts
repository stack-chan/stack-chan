...
test('USB speaker playback keeps one second of PCM and starts with a 500 ms prebuffer', () => {
  ...
  assert.match(bridgeSource, /const USB_RX_WINDOW_BYTES = 12 \* 1024/)
  assert.match(bridgeSource, /const SPEAKER_CREDIT_WINDOW_BYTES = USB_RX_WINDOW_BYTES/)
  assert.match(bridgeSource, /#readBuffer = new Uint8Array\(USB_RX_WINDOW_BYTES\)/)
