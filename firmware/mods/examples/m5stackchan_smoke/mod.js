...
async function runSmoke(robot) {
  ...
  try {
    ...
    await robot.motion.setPose({ rotation: { y: 0, p: 0, r: 0 } }, 0.5)
    await delay(STEP_MS)
    trace('[M5StackChan CoreS3 smoke] servo: small yaw/pitch check\n')
    await robot.motion.setPose({ rotation: { y: 0.08, p: -0.06, r: 0 } }, 0.5)
    await delay(STEP_MS)
    trace('[M5StackChan CoreS3 smoke] servo: neutral pose\n')
    await robot.motion.setPose({ rotation: { y: 0, p: 0, r: 0 } }, 0.5)
  } catch (error) {
  ...
  robot.lighting.lightOn(LED_NAME, 24, 0, 0)
  await delay(STEP_MS)
  trace('[M5StackChan CoreS3 smoke] LED: lightBlink green\n')
  robot.lighting.lightBlink(LED_NAME, 0, 24, 0, 250)
  await delay(STEP_MS * 2)
  trace('[M5StackChan CoreS3 smoke] LED: lightRainbow\n')
  robot.lighting.lightRainbow(LED_NAME)
  await delay(STEP_MS * 2)
