...
async function main(startupScreen: StartupScreen) {
  ...
  const forceSetup = startupScreen.isSetupRequested()
  startupScreen.setStatus(forceSetup ? 'Opening setup screen...' : 'Running launch hooks...')
  const shouldRobotCreate = await (onLaunch?.({ forceSetup }) ?? true)
...
const startupScreen = new StartupScreen({
  onOpenSetup: () => {
    trace('[startup] setup screen requested from startup UI\n')
  },
})
