...
function makeAppHeader(version = '9.5.0+stackchan.1', projectName = 'xs_esp32') {
...
test('removeModFromDevice clears the first xs sector and reboots', async () => {
  ...
  assert.equal(preflight.firmware.version, '9.5.0+stackchan.1')
  assert.equal(result.verified, true)
  assert.equal('backup' in result, false)
})
