...
import { type InstalledMod, type SimulatorModResult } from '@/services/simulator/simulator-engine.mjs'
...
export function SimulatorPage() {
  ...
  const modBusy = simulator.modState.result.status === 'restarting'
  const cameraBusy = simulator.cameraStatus.status === 'pending'
  const cameraLabel = {
    idle: '未接続',
    pending: '接続中',
    connected: '接続済み',
    ...
    error: `接続失敗 · 合成映像`,
  }[simulator.cameraStatus.status]

  return (
    <div className="page-container grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="grid min-w-0 gap-4">
        <SimulatorSurface viewportRef={simulator.viewportRef} screenRef={simulator.screenRef} />
        <OperationStatus
          state={simulator.operation}
          labels={{
        ...
        />
        <LogConsole entries={simulator.logs} onClear={simulator.clearLogs} title={t('ファームウェアログ')} />
      </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              ...
              <Badge variant={simulator.modState.result.status === 'error' ? 'destructive' : 'secondary'}>
                {simulator.modState.result.status}
              </Badge>
            </div>
          </CardHeader>
          <CardContent className="grid gap-3">
            <input
              ...
              onChange={(event) => {
                ...
                if (file) void simulator.installMod(file)
            ...
            <p className="min-h-5 text-sm text-muted-foreground" role="status">
              {t(describeMod(simulator.modState.result, simulator.modState.installedMod))}
            </p>
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" onClick={() => void simulator.restart()} disabled={modBusy}>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction variant="destructive" onClick={() => void simulator.clearMod()}>
                      {t('削除')}
                    </AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-3 gap-2">
            {(['a', 'b', 'c'] as const).map((name) => (
              <Button key={name} variant="outline" onClick={() => simulator.pushButton(name)}>
                {name.toUpperCase()}
              </Button>
            ))}
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <Button
              className="w-full"
              variant="outline"
              onClick={() => void simulator.connectCamera()}
              disabled={cameraBusy}
            >
